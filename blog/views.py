...
def post_create(request):
	...
	return render(request, 'blog/post/create.html', {'form':form,'user':user})
...
def post_share(request, post_id):
	...
	if request.method == 'POST':
		...
		if form.is_valid():
			...
			subject = '{} ({}) recommends that you reading "{}"'.format(cd['name'], cd['email'], post.title)
			message = 'Read "{}" at {}\n\n{}\'s comments: {}'.format(post.title, post_url, cd['name'], cd['comments'])
			to = cd['to']
			name = cd['name']
			send_mail(subject, message, 'vagrant@myblog.com', [cd['to']])
...
def post_list(request, tag_slug=None):
	...
	try:
		...
		#if not an integer return the first page
